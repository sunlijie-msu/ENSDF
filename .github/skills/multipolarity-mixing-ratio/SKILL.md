...
- **δ = 0 **: Pure transition (single multipolarity, e.g., pure E1) MR field is blank.
- **δ > 0 or δ < 0**: Mixed transition (higher multipolarities contributing)
- **δ compatible with 0**: Lower-order transition dominant with small (including negligible) contributions from higher-order multipoles.
...
- **Always include sign** in MR field (+ or -) except for MR=0.0
