#!/usr/bin/env python3
"""
CORRECTED VERIFICATION: Handle integer vs decimal energies properly
Integer energies: DE field = uncertainty value directly
Decimal energies: DE field = uncertainty × 10
"""
...
def main():
    ...
    print("CORRECTED VERIFICATION: Proper Integer vs Decimal Energy Handling")
    print("=" * 70)
    print("Energy   | Energy Type | File DE | Ref dEx | Expected DE | Status      | Line")
    print("-" * 70)
    ...
    for ref_energy in sorted(reference_data.keys()):
        ...
        if ref_energy in current_lookup:
            ...
            energy_str = record['energy_str']
            
            # CORRECTED LOGIC: Check if energy has decimal places
            is_decimal = '.' in energy_str
            energy_type = "Decimal" if is_decimal else "Integer"
            
            # Convert reference uncertainty to ENSDF DE field value
            if is_decimal:
                # Decimal energies: multiply uncertainty by 10 for DE field
                expected_de = int(ref_uncertainty * 10)
            else:
                # Integer energies: uncertainty value directly
            ...
            print(f"{ref_energy:7.1f} | {energy_type:11s} | {current_de:7s} | {ref_uncertainty:6.1f} | {expected_de_clean:10s} | {status:10s} | {record['line']:4d}")
        else:
            status = "MISSING"
            missing_levels.append(ref_energy)
            print(f"{ref_energy:7.1f} | {'Unknown':11s} | {'N/A':7s} | {ref_uncertainty:6.1f} | {'N/A':10s} | {status:10s} | N/A")
    ...
    if incorrect_de:
        ...
        for fix in incorrect_de:
            print(f"  Line {fix['line']:3d}: {fix['energy']:7.1f} keV ({fix['energy_type']}) - DE '{fix['current_de']}' should be '{fix['expected_de']}'")
    
    if not missing_levels and not incorrect_de:
        print("\nSUCCESS: All 33 reference levels found with correct DE values!")
        print("ENSDF format compliance: Integer and decimal energies handled properly")
    else:
