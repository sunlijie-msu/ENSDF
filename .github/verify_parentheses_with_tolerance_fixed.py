...
with energy tolerance matching.
"""
...
def extract_xundl_levels(filename):
    """Extract level data from XUNDL file"""
    levels = []
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            content = f.read()
        ...
        # Find L-records with energy and J-π
        l_records = re.findall(r'^127I\s+L\s+(\d+(?:\.\d+)?)\s+\d*\s+([^\s]+.*?)(?:\s+[A-Z])?$', content, re.MULTILINE)
    ...
    for ensdf_energy, ensdf_jpi in ensdf_levels:
        ...
        if xundl_energy is not None:
            if ensdf_jpi == xundl_jpi:
                ...
                print(f"  ENSDF: '{ensdf_jpi}' vs XUNDL: '{xundl_jpi}'")
