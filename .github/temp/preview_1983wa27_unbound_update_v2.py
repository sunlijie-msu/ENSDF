...
out_path = Path(r'd:\X\ND\ENSDF\.github\temp\1983WA27_Unbound_preview_v2.csv')
...
for row in rows[2:]:
    if len(row) > 1 and row[1] in ei_map:
        row[1] = ei_map[row[1]]
    if row:
        field = row[-1]
        def repl(match):
            ...
            return token + '('
        row[-1] = re.sub(r'\b(\d+(?:\.\d+)?)\(', repl, field)
...
for token in sorted(unresolved):
    count = 0
    for row in rows[2:]:
        if row and token + '(' in row[-1]:
            count += row[-1].count(token + '(')
    print(token, count)
