#!/usr/bin/env python3
"""
Generate ENSDF L and G records from 2001VO24.csv
Reads CSV, extracts transitions, generates properly formatted 80-column records
"""
...
if __name__ == '__main__':
    ...
    print("GENERATED ENSDF RECORDS (sorted by Exi):\n")
    for record in records:
        print(record)
    
    print("\n" + "="*80)
    print("VERIFICATION CHECKS:")
    print(f"Total L-records: {len(transitions)}")
    print(f"Total G-records: {sum(len(v) for v in transitions.values())}")
    print(f"Total lines: {len(records)}")
    
    # Verify line lengths
    all_80 = all(len(r) == 80 for r in records)
    print(f"All lines exactly 80 chars: {all_80}")
