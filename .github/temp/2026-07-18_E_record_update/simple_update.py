"""
Simple E-record update: replace IB/DIB/IE/DIE/TI/DTI fields in original ENSDF.
Uses clean original from git (temp/orig.ens), applies Table III data.
Each E-record line: only chars at pos 21-27,28-29,30-37,38-39,64-73,74-75 are touched.
"""
import re

ORIG_PATH = 'd:/X/ND/ENSDF/.github/temp/orig.ens'
T3_PATH   = 'd:/X/ND/ENSDF/XUNDL/2026OSAA_CT11035_152Gd_Table_III.md'
OUT_PATH  = 'd:/X/ND/ENSDF/XUNDL/2026OSAA_CT11035_152Gd.ens'
...
def fmt_val(val, w):
    ...
    # shorten E-n mantissa if overflow
    em = re.match(r'^([+\-]?\d+\.?\d*)(E[+\-]\d+)$', s)
    if em:
        m, e = em.group(1), em.group(2)
        max_m = w - len(e)
        mv = float(m)
        for dp in range(max_m-1, -1, -1):
            short = str(int(round(mv))) if dp==0 else f'{mv:.{dp}f}'
            if len(short+e) <= w: return (short+e).ljust(w)
    return s[:w].ljust(w)
...
for line in lines:
    ...
    if len(r) >= 9 and r[5:7] == '  ' and r[7:8] == 'E' and cur_e_key is not None and cur_e_key in t3:
        ...
        # Build new line: replace only the 6 fields, everything else unchanged
        ib  = fmt_val(d['ib'] if not d['ibb'] else '', 7)    # pos 21-27
        dib = fmt_unc(d['ibu'] if not d['ibb'] else '', 2)   # pos 28-29
        ie  = fmt_val(d['ie'] if not d['ieb'] else '', 8)    # pos 30-37
        die = fmt_unc(d['ieu'] if not d['ieb'] else '', 2)   # pos 38-39
        ti  = fmt_val(d['it'] if not d['itb'] else '', 10)   # pos 64-73
        dti = fmt_unc(d['itu'] if not d['itb'] else '', 2)   # pos 74-75
        
        new_r = (r[:21] + ib + dib + ie + die + r[40:64] + ti + dti + r[76:])
        
        if new_r != r:
            new_lines.append(new_r + '\n')
            modified += 1
        else:
...
print(f"Updated {modified} E-records.")
