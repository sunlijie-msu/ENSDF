"""Show FULL J$ comments (no truncation) plus raw L-record lines for each level 200-5200 keV."""
        ...
        if 200 <= e_val <= 5200:
            ...
            while j < len(lines):
                ...
                if is_cL:
                    ...
                    elif in_jcomment and not (len(content) >= 2 and content[1] == '$'):
...
for e, ln, jpi, jcomment in results:
    flag = ' <<< NO J$' if not jcomment else ''
    print(f'L{ln:4d} E={e:8.2f}  J={jpi}')
    if jcomment:
        print(f'       J$={jcomment}')
    print(flag if flag else '', end='')
    print()
